# Dorn's Pose Controls v0.1.0

Direct pose keybinds for **S.T.A.L.K.E.R. G.A.M.M.A.**

## Behavior

Changing pose (stand/crouch/prone/walk) is now a 1 to 1 mapping.

- **Press crouch key** always normal crouch.
- **Press prone key (walk/low-crouch)** always go prone (low crouch).
- **Walk key (new bind for this mod)** always walk (means standing if not).
- **Sprint key** unchanged (stand + sprint)

## Installation

1. Download the latest release from https://github.com/JoshuaCarter/GAMMA-Pose-Controls/releases
2. Install via MO2 like normal
3. **(Required)** Your crouch/walk hold vs toggle settings must match, else this mod will do nothing and spam you with PDA warning messages.
4. (Optional) assign walk key

## Other mods

- https://github.com/JoshuaCarter/GAMMA-Dont-ReEquip-PDA
- https://github.com/JoshuaCarter/GAMMA-Faster-Skinning
- https://github.com/JoshuaCarter/GAMMA-Field-Strip-All-Keybind
- https://github.com/JoshuaCarter/GAMMA-FPS-Counter
- https://github.com/JoshuaCarter/GAMMA-Giant-Jump
- https://github.com/JoshuaCarter/GAMMA-Optimal-Tool-Use
- https://github.com/JoshuaCarter/GAMMA-Prone-Fix
- https://github.com/JoshuaCarter/GAMMA-Radiation-Grain-Fix
- https://github.com/JoshuaCarter/GAMMA-Tooltip-Weapon-Damage
